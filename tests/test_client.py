...
class FlaskClientTestCase(unittest.TestCase):
    ...
    def test_register_and_login(self):
        ...
        user=User.query.filter_by(eamil='john@example.com').first()
        token=user.generate_confirmation_token()
        response=self.client.get(url_for('auth.confirm',token=token),follow_redirect=True)
        data=response.get_data(as_text=True)
        self.assertTrue('You have confirmed your account' in response.get_data(as_text=True))

        response=self.client.get(url_for('auth.logout'),follow_redirect=True)
        data=response.get_data(as_text=True)
